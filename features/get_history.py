...
def get_history():
    ...
    query = "SELECT * FROM history  ORDER BY created_at DESC LIMIT 20"
...
def calculate_average_growth(fishes):
    ...
    first_date = fishes[-1]['created_at']
    last_date = fishes[0]['created_at']
    num_days = (last_date - first_date).days

    first_weight = float(fishes[-1]['weight'])
    last_weight = float(fishes[0]['weight'])

    average_growth = (last_weight - first_weight) / num_days
    length_carp_average = round((fishes[0]['length_carp'] - fishes[-1]['length_carp']) / num_days, 1)
    length_tilapia_average = round((fishes[0]['length_tilapia'] - fishes[-1]['length_tilapia']) / num_days, 1)
    length_catfish_average = round((fishes[0]['length_catfish'] - fishes[-1]['length_catfish']) / num_days, 1)
    length_perch_average = round((fishes[0]['length_perch'] - fishes[-1]['length_perch']) / num_days, 1)
    weight_carp_average = round((fishes[0]['weight_carp'] - fishes[-1]['weight_carp']) / num_days, 1)
    weight_tilapia_average = round((fishes[0]['weight_tilapia'] - fishes[-1]['weight_tilapia']) / num_days, 1)
    weight_catfish_average = round((fishes[0]['weight_catfish'] - fishes[-1]['weight_catfish']) / num_days, 1)
    weight_perch_average = round((fishes[0]['weight_perch'] - fishes[-1]['weight_perch']) / num_days, 1)
...
def predict_future_weight(target_date):
    ...
    last_weight = float(fishes[0]['weight'])
    last_length_carp = float(fishes[0]['length_carp'])
    last_length_tilapia = float(fishes[0]['length_tilapia'])
    last_length_catfish = float(fishes[0]['length_catfish'])
    last_length_perch = float(fishes[0]['length_perch'])
    last_weight_carp = float(fishes[0]['weight_carp'])
    last_weight_tilapia = float(fishes[0]['weight_tilapia'])
    last_weight_catfish = float(fishes[0]['weight_catfish'])
    last_weight_perch = float(fishes[0]['weight_perch'])
